(function() {
	...
	techniqueFactory.$inject = ['lockFactory'];

	function techniqueFactory(lockFactory) {
		//Variables
		var fluExtra = readFluExtra();
		var labTechniques = readLabTechniques();
		var researchTechniques = readResearchTechniques();
		var revisionTechniques = readRevisionTechniques();
		var usefulTechniques = readUsefulTechniques();
		...
		var factory = {
			...
			readUsefulTechniques: readUsefulTechniques,
		...
		function readTechniques(showRevisionOnly, showLabOnly) { 
			...
			for(var i = 0; i < techniques.length; i++) {
				...
				if((!techniques[i].revisionOnly && !techniques[i].labOnly) || ((techniques[i].revisionOnly === showRevisionOnly) && (techniques[i].labOnly === showLabOnly))) {
